...
// TestBasic ensures that there is a ready azure-cloud-controller-manager-app daemonset.
func TestBasic(t *testing.T) {
	ctx := context.Background()
	var err error

	// Check azure-cloud-controller-manager-app deamonset is ready.
	err = checkReadyDaemonset(ctx)
	if err != nil {
		t.Fatalf("could not get azure-cloud-controller-manager-app: %v", err)
	}
}
